...
def propose_for_workspace(
    ...
    *,
    ...
    """Profile, score, and write proposals. Returns what it proposed.

    `dry_run` scores without writing, which is how this should be run the first
    time against any real catalog: the output is a review queue a person will
    have to work through, and one badly-tuned threshold turns it into a queue
    nobody works through at all.

    Writing goes through `catalog.propose_relationship`, which refuses a
    suppressed pair and refuses to overwrite anything a person asserted. Those
    checks are deliberately NOT repeated here - a suppression that only holds
    when the caller remembers to look is not a suppression.
    """
