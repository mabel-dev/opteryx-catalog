...
def entry_to_dict(entry: ManifestEntry) -> Dict[str, Any]:
    ...
    if df.partition:
        partition_json = json.dumps({
            k: _serialize_value(v) for k, v in df.partition.items()
        })
...
def write_parquet_manifest(
    ...
    logger.debug(
        f"Collected {len(all_entries)} data file entries from {manifest_count} manifests"
    )
...
def read_parquet_manifest(
    ...
    try:
        ...
        logger.debug(
            f"Read Parquet manifest: {len(table)} files from {parquet_path}"
        )
        ...
        logger.warning(f"Failed to read Parquet manifest from {parquet_path}: {exc}, falling back to Avro")
        return None
...
class OptimizedDataScan(DataScan):
    ...
    def plan_files(self) -> Iterable[FileScanTask]:
        ...
        if parquet_records is not None:
            ...
            message = (
                f"Query planning: ✓ PARQUET manifest ({len(parquet_records)} files, {elapsed:.1f}ms read)"
            )
            print(message)
            logger.info(
